...
class conv_lstm(nn.Module):
    def __init__(self, hidden_size, kernel, stride, nb_filter, input_size):
        ...
        self.lstm = create_model(model='skip_lstm',input_size=nb_filter,
...
class Scoring(nn.Module):
    def __init__(self, feature_size):
        ...
        conv_input = 128
        self.conv = nn.Sequential(nn.Conv1d(feature_size, conv_input, 1, 1),
                                  nn.ReLU(),
    ...
    def forward(self, model_input):
        ...
        model_input = self.conv.forward(model_input).permute(0, 2, 1)  # batch_size * 300 * hidden_size(128)
        penal = None
        attn, penal = self.attn.forward(model_input)
        attn, _ = self.lstm(attn)
        attn = attn[:, -1, :]

        M_output = torch.cat([self.relu(self.linear_skip1(self.scale1(model_input))),
                              self.relu(self.linear_skip2(self.scale2(model_input))),
                              self.relu(self.linear_skip3(self.scale3(model_input)))], 1)

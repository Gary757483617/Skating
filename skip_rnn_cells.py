    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            for idx in range(self._num_layers):
                with tf.variable_scope('layer_%d' % (idx + 1)):
                    ...
                    if self._layer_norm:
                        ...
                        j = rnn_ops.layer_norm(j, name="j")
                        f = rnn_ops.layer_norm(f, name="f")
                        o = rnn_ops.layer_norm(o, name="o")   # eq. (8)(9) without sigmoid

                    new_c_tilde = (c_prev * tf.sigmoid(f + self._forget_bias) + tf.sigmoid(i) * self._activation(j))  # eq. (10)
                    new_h_tilde = self._activation(new_c_tilde) * tf.sigmoid(o)  # eq. (11)
            ...
            with tf.variable_scope('state_update_prob'):
                ...
                new_update_prob_tilde = tf.sigmoid(new_update_prob_tilde)   # eq. (14)

            # Compute value for the update gate
            cum_update_prob = cum_update_prob_prev + tf.minimum(update_prob_prev, 1. - cum_update_prob_prev)  # eq.(15)后半部分
            update_gate = _binary_round(cum_update_prob)  # eq.(7)

            # Apply update gate
            new_states = []
            for idx in range(self._num_layers - 1):
                new_c = update_gate * state_candidates[idx].c + (1. - update_gate) * state[idx].c  # eq. (12)
                new_h = update_gate * state_candidates[idx].h + (1. - update_gate) * state[idx].h  # eq. (13)
                new_states.append(LSTMStateTuple(new_c, new_h))
            new_c = update_gate * state_candidates[-1].c + (1. - update_gate) * state[-1].c  # eq. (12)
            new_h = update_gate * state_candidates[-1].h + (1. - update_gate) * state[-1].h  # eq. (13)
            new_update_prob = update_gate * new_update_prob_tilde + (1. - update_gate) * update_prob_prev
            new_cum_update_prob = update_gate * new_update_prob + (1. - update_gate) * cum_update_prob  # eq. (15)合并
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            for idx in range(self._num_layers):
                with tf.variable_scope('layer_%d' % (idx + 1)):
                    ...
                    # Parameters of gates are concatenated into one multiply for efficiency.
                    with tf.variable_scope("gates"):
                        concat = rnn_ops.linear([cell_input, h_prev], 2 * self._num_units[idx], bias=True,
                                                bias_start=1.0, )
